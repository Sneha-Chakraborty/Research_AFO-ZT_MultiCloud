...
def main() -> None:
    ...
    ap.add_argument(
        ...
        choices=["static"],
        default="static",
        help="Which baseline to run (Step 3 implements only: static).",
    )
    ap.add_argument("--in", dest="in_csv", default=str(DEFAULT_UNIFIED_CSV), help="Input unified telemetry CSV.")
    ap.add_argument("--out", dest="out_csv", default=str(RESULTS_DIR / "baseline_static_scores.csv"), help="Output scores CSV.")
    ap.add_argument("--model-out", default=str(MODELS_DIR / "baseline_static.pkl"), help="Where to store baseline artifact.")
    args = ap.parse_args()
